...
def concat_by_id_list_with_note_chunk_id_time(df, labels, inputs, masks, note_ids, times, str_len):
    ...
    for id in id_lists:
        ...
        final_times.append(torch.tensor(np.concatenate([np.zeros(1, dtype=np.float32),np.diff(times[id_ix]).astype(np.float32)]),dtype=torch.float32,))
...
def write_performance(flat_true_labels, flat_predictions, flat_logits, config, max_seq_length, data_dir):
    ...
    test_msl = max_seq_length

    test_dir_code = data_dir.split('_')[-1]
