...
def main():
    ...
    parser.add_argument("--train_data",
                        ...
                        type=str,
                        required=True,
                        help="The input training data file name. Should be the .tsv file (or other data file) for the task.")

    parser.add_argument("--val_data",
                        default=None,
                        type=str,
                        required=True,
                        help="The input validation data file name. Should be the .tsv file (or other data file) for the task.")

    parser.add_argument("--test_data",
                        default=None,
                        type=str,
                        required=True,
                        help="The input test data file name. Should be the .tsv file (or other data file) for the task.")
    ...
    parser.add_argument("--max_seq_length",
                        default=128,
                        type=int,
                        help="The maximum total input sequence length after WordPiece tokenization. Sequences longer than this will be truncated, and sequences shorter will be padded.")
    ...
    for i in range(n_gpu):
        write_log(("Device Name: {}," "Device Capability: {}").format(torch.cuda.get_device_name(i),
    ...
    num_train_steps = int(len(train_labels) / args.gradient_accumulation_steps * args.num_train_epochs)
    ...
    with torch.autograd.set_detect_anomaly(False):
        for epoch in trange(epochs, desc="Epoch"):
            ...
            for step in range(ev_ids_num):
                with torch.no_grad():
                    b_input_ids, b_labels, b_input_mask, b_note_ids, b_chunk_ids, b_times = next(validation_batch_generator)
                    b_input_ids = b_input_ids.to(device)
                    b_input_mask = b_input_mask.to(device)
                    b_new_note_ids = convert_note_ids(b_note_ids).to(device)
                    b_chunk_ids = b_chunk_ids.unsqueeze(0).to(device)
            ...
            output_checkpoints_path = os.path.join(args.output_dir, "bert_fine_tuned_with_note_checkpoint_%d.pt" % epoch)
    ...
    if args.save_model:
        ...
        output_fig_path = os.path.join(args.output_dir, "bert_fine_tuned_with_note_training_loss_{}_{}.png".format(args.seed, args.data_dir.split('_')[-1]))
        plt.savefig(output_fig_path, dpi=fig1.dpi)
    ...
    if args.save_model:
        ...
        output_df.to_csv(os.path.join(args.output_dir, 'test_predictions_{}_{}.csv'.format(args.seed, args.data_dir.split('_')[-1])), index=False)
